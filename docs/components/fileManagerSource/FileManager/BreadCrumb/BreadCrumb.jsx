...
import { LuHouse, LuEllipsis, LuChevronRight, LuPanelLeftOpen, LuPanelLeftClose } from "react-icons/lu";
      ...
      <div className="breadcrumb" ref={breadCrumbRef}>
        ...
        {folders.map((folder, index) => (
          <div key={index} style={{ display: "contents" }}>
            ...
            >
              {index === 0 ? <LuHouse /> : <LuChevronRight />}
              {folder.name}
            </span>
            {hiddenFolders?.length > 0 && index === 0 && (
              ...
              >
                <LuEllipsis size={14} className="hidden-folders" />
              </button>
            )}
          </div>
        ))}
