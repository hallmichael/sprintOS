...
import {Adapt, AnimatePresence, Button, Popover, Text, View, createStyledContext, isWeb, styled, withStaticProperties} from 'tamagui';
...
export const DatePickerInput = Input.Area.styleable<DatePickerInputProps>((props, ref) => {
  ...
  return (
    <View $platform-native={{minWidth: '100%'}}>
      <Input size={size}>
        <Input.Box bc="$inputBorderDefault" bw="$0.5" br="$4">
          <Input.Icon bg="$inputBgDefault">
            <Calendar color="$iconSecondary" />
          </Input.Icon>
          <Input.Section>
            <Input.Area value={value} ref={ref} {...rest} color="$textNeutral" bw="$0" />
          </Input.Section>
          <Input.Section>
            <Input.Button
              bg="$inputBgDefault"
